...
for n in range(4):
	...
	classifierRF = RandomForestClassifier(n_estimators =200,class_weight=None,max_depth=50,min_samples_split=7,min_samples_leaf=3,criterion = 'entropy',n_jobs=-1,oob_score=True)
	...
	#XGBoost classifier
	classifierXGB = XGBClassifier(n_estimators=200,n_jobs=-1)
	...
	classifierGB=GradientBoostingClassifier(n_estimators=100,learning_rate=1.0,max_depth=1).fit(x_train, y_train)
        pred = classifierGB.predict(X_ul)


	#Adaboost classifier
	classifierAB = AdaBoostClassifier(base_estimator=RandomForestClassifier(n_estimators = 200,class_weight=class_weights,max_depth=50,min_samples_split=7,
                                          min_samples_leaf=3, criterion = 'entropy',n_jobs=-1,oob_score=True),n_estimators = 200)
	...
	classifierCB=CatBoostRegressor(iterations=50, depth=5, learning_rate=0.1, loss_function='RMSE',silent=True)
	classifierCB.fit(X_train, y_train,eval_set=(x_test, y_test))
	...
	classifierVC = VotingClassifier(estimators=[('rf', classifierRF),('svm', classifierSVM),('lr',classifierLR ), 
                                            ('nb', classifierNB),('knn', classifierKNN),('bg', classifierBG),
                                            ('xgb',classifierXGB),('gb', classifierGB),('ab', classifierAB)], 
	...
	for clf, label in zip([classifierRF,classifierSVM,classifierLR,classifierNB,classifierKNN,classifierBG,classifierXGB,classifierGB,classifierAB,classifierVC],
	    	...
	    	scores = cross_val_score(clf, X, [round(i) for i in y], cv=20, scoring='accuracy')
